...
- 固定工作量收益实验可用 `AEGISAI_TCB_EXECUTOR_WORK_UNITS` 和
  `AEGISAI_TCB_WORKER_WORK_UNITS`，避免固定墙钟 busy-loop 抹平 scheduler 收益；
  baseline-only affinity 可用 `AEGISAI_TCB_BASELINE_EXECUTOR_AFFINITY_CPUS` 和
  `AEGISAI_TCB_BASELINE_BACKGROUND_AFFINITY_CPUS` 固定干扰形状
