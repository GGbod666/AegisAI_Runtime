...
PROFILE="${AEGISAI_TCB_PROFILE:-default}"
case "${PROFILE}" in
  default)
    PROFILE_EXECUTOR_CPU_MS=1800
    PROFILE_WORKER_CPU_MS=2600
    PROFILE_EXECUTOR_WORK_UNITS=0
    PROFILE_WORKER_WORK_UNITS=0
    PROFILE_WORKER_IO_KB=256
    PROFILE_WORKER_START_DELAY_MS=50
    PROFILE_EXECUTOR_AFFINITY_CPUS=""
    PROFILE_BACKGROUND_AFFINITY_CPUS=""
    PROFILE_BASELINE_EXECUTOR_AFFINITY_CPUS=""
    PROFILE_BASELINE_BACKGROUND_AFFINITY_CPUS=""
    PROFILE_DAEMON_MAX_EVENTS=64
    PROFILE_LIVE_PID_DERIVE_TIMEOUT_MS=1000
    PROFILE_LIVE_ENABLE_AFFINITY=""
    PROFILE_RUN_DRY_RUN=1
    PROFILE_ROUNDS=3
    PROFILE_MODES=""
    PROFILE_MIN_BENEFIT_PCT=5
    PROFILE_REQUIRE_BENEFIT=0
    ;;
  fixed_work_guarded)
    PROFILE_EXECUTOR_CPU_MS=1
    PROFILE_WORKER_CPU_MS=1
    PROFILE_EXECUTOR_WORK_UNITS=6000000
    PROFILE_WORKER_WORK_UNITS=10000000
    PROFILE_WORKER_IO_KB=0
    PROFILE_WORKER_START_DELAY_MS=0
    PROFILE_EXECUTOR_AFFINITY_CPUS=""
    PROFILE_BACKGROUND_AFFINITY_CPUS="0,1"
    PROFILE_BASELINE_EXECUTOR_AFFINITY_CPUS="0,1"
    PROFILE_BASELINE_BACKGROUND_AFFINITY_CPUS="0,1"
    PROFILE_DAEMON_MAX_EVENTS=512
    PROFILE_LIVE_PID_DERIVE_TIMEOUT_MS=1500
    PROFILE_LIVE_ENABLE_AFFINITY=1
    PROFILE_RUN_DRY_RUN=1
    PROFILE_ROUNDS=3
    PROFILE_MODES="baseline,live_guarded"
    PROFILE_MIN_BENEFIT_PCT=5
    PROFILE_REQUIRE_BENEFIT=1
    ;;
  *)
    printf 'ERROR: unknown AEGISAI_TCB_PROFILE: %s\n' "${PROFILE}" >&2
    exit 2
    ;;
esac
...
EXECUTOR_CPU_MS="${AEGISAI_TCB_EXECUTOR_CPU_MS:-${PROFILE_EXECUTOR_CPU_MS}}"
WORKER_CPU_MS="${AEGISAI_TCB_WORKER_CPU_MS:-${PROFILE_WORKER_CPU_MS}}"
EXECUTOR_WORK_UNITS="${AEGISAI_TCB_EXECUTOR_WORK_UNITS:-${PROFILE_EXECUTOR_WORK_UNITS}}"
WORKER_WORK_UNITS="${AEGISAI_TCB_WORKER_WORK_UNITS:-${PROFILE_WORKER_WORK_UNITS}}"
WORKER_IO_KB="${AEGISAI_TCB_WORKER_IO_KB:-${PROFILE_WORKER_IO_KB}}"
WORKER_START_DELAY_MS="${AEGISAI_TCB_WORKER_START_DELAY_MS:-${PROFILE_WORKER_START_DELAY_MS}}"
EXECUTOR_AFFINITY_CPUS="${AEGISAI_TCB_EXECUTOR_AFFINITY_CPUS:-${PROFILE_EXECUTOR_AFFINITY_CPUS}}"
BACKGROUND_AFFINITY_CPUS="${AEGISAI_TCB_BACKGROUND_AFFINITY_CPUS:-${PROFILE_BACKGROUND_AFFINITY_CPUS}}"
if [[ -n "${AEGISAI_TCB_BASELINE_EXECUTOR_AFFINITY_CPUS:-}" ]]; then
  BASELINE_EXECUTOR_AFFINITY_CPUS="${AEGISAI_TCB_BASELINE_EXECUTOR_AFFINITY_CPUS}"
elif [[ -n "${PROFILE_BASELINE_EXECUTOR_AFFINITY_CPUS}" ]]; then
  BASELINE_EXECUTOR_AFFINITY_CPUS="${PROFILE_BASELINE_EXECUTOR_AFFINITY_CPUS}"
else
  BASELINE_EXECUTOR_AFFINITY_CPUS="${EXECUTOR_AFFINITY_CPUS}"
fi
if [[ -n "${AEGISAI_TCB_BASELINE_BACKGROUND_AFFINITY_CPUS:-}" ]]; then
  BASELINE_BACKGROUND_AFFINITY_CPUS="${AEGISAI_TCB_BASELINE_BACKGROUND_AFFINITY_CPUS}"
elif [[ -n "${PROFILE_BASELINE_BACKGROUND_AFFINITY_CPUS}" ]]; then
  BASELINE_BACKGROUND_AFFINITY_CPUS="${PROFILE_BASELINE_BACKGROUND_AFFINITY_CPUS}"
else
  BASELINE_BACKGROUND_AFFINITY_CPUS="${BACKGROUND_AFFINITY_CPUS}"
fi
...
DAEMON_MAX_EVENTS="${AEGISAI_TCB_DAEMON_MAX_EVENTS:-${PROFILE_DAEMON_MAX_EVENTS}}"
DAEMON_DRAIN_MS="${AEGISAI_TCB_DAEMON_DRAIN_MS:-1200}"
DAEMON_TICK_MS="${AEGISAI_TCB_DAEMON_TICK_MS:-100}"
LIVE_PID_DERIVE_TIMEOUT_MS="${AEGISAI_TCB_LIVE_PID_DERIVE_TIMEOUT_MS:-${PROFILE_LIVE_PID_DERIVE_TIMEOUT_MS}}"
...
LIVE_ENABLE_AFFINITY="${AEGISAI_ENABLE_LIVE_AFFINITY:-${PROFILE_LIVE_ENABLE_AFFINITY}}"
...
RUN_DRY_RUN="${AEGISAI_TCB_RUN_DRY_RUN:-${PROFILE_RUN_DRY_RUN}}"
ROUNDS="${AEGISAI_TCB_ROUNDS:-${PROFILE_ROUNDS}}"
if [[ -n "${AEGISAI_TCB_MODES:-}" ]]; then
  MODES="${AEGISAI_TCB_MODES}"
elif [[ -n "${PROFILE_MODES}" ]]; then
  MODES="${PROFILE_MODES}"
...
MIN_BENEFIT_PCT="${AEGISAI_TCB_MIN_BENEFIT_PCT:-${PROFILE_MIN_BENEFIT_PCT}}"
REQUIRE_BENEFIT="${AEGISAI_TCB_REQUIRE_BENEFIT:-${PROFILE_REQUIRE_BENEFIT}}"
...
Common overrides:
  AEGISAI_TCB_PROFILE=default|fixed_work_guarded
...
The fixed_work_guarded profile applies the verified CPU-affinity benefit
shape: fixed hash work units, background contention on CPUs 0,1, baseline
critical-chain affinity on CPUs 0,1, live guarded nice+affinity, and a hard
benefit gate. It still requires AEGISAI_CONFIRM_LIVE_ACTUATOR=1.
USAGE
}
